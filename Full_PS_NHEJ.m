function [newflog, P_time_D, P_time_W, newflogr, newPSD, newPSW]  = Full_PS(num_files, hval, sval, epsilonval, Ccval, nuval)
...
format_nu = sprintf('%.3f', round_nu);    
    ...
    for i = 1:num_files
        ...
        file_name1 = sprintf("%s/SIMS_NHEJ/TM_fem_pop_%d_h=%s_s=%s_ep=%s_Cc=%s_nu=%s.mat", home_directory, i, format_h, format_s, format_ep, format_Cc, format_nu)
        load(file_name1);

        file_name2 = sprintf("%s/SIMS_NHEJ/TM_male_pop_%d_h=%s_s=%s_ep=%s_Cc=%s_nu=%s.mat", home_directory, i, format_h, format_s, format_ep, format_Cc, format_nu);
        load(file_name2);
        
        file_name3 = sprintf("%s/SIMS_NHEJ/TM_pop_%d_h=%s_s=%s_ep=%s_Cc=%s_nu=%s.mat", home_directory, i, format_h, format_s, format_ep, format_Cc, format_nu);
        load(file_name3);
